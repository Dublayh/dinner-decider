...
- **Meal plan → grocery list** — Opening the shopping list from meal plan pushes that week's recipe ingredients into `grocery_list` (skipping duplicates by `${text}|${source}` key), then opens the modal.
